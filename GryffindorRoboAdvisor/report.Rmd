...
if (is.null(input$industry1)) {
        ...
        cat("The industries that have been chosen to exclude are:", input$industry1, sep = ", ")
}
```
...
```{r echo = FALSE, results='asis'}
if (input$rpref2 == 1 && input$inv_horizon <= 5) {
  cat("You have chosen a risk preference of level", input$rpref2, ", corresponding to a yearly
      SD of XY, and an investment horizon of", input$inv_horizon, "years. According to these
...
specified, you can expect a terminal wealth of `r round(((1+(averagereturn(portfoliofinal)))^input$inv_horizon)*input$initial_wealth)`.
